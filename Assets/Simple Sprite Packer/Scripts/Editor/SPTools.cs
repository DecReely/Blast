...
namespace SimpleSpritePackerEditor
{
	public class SPTools
	{
		...
		public static bool ImportAndConfigureAtlasTexture(Texture2D targetTexture, Texture2D sourceTexture, Rect[] uvs, SPSpriteImportData[] spritesImportData)
		{
...
#if UNITY_2021_2_OR_NEWER
			// Configure the sprites meta data
			SpriteDataProviderFactories factory = new SpriteDataProviderFactories();
            factory.Init();

            ISpriteEditorDataProvider dataProvider = factory.GetSpriteEditorDataProviderFromObject(texImporter);
			...
			List<SpriteRect> newSpriteRects = new();
            List<SpriteNameFileIdPair> nameFileIdPairs = spriteNameFileIdDataProvider.GetNameFileIdPairs().ToList();
			List<SpriteNameFileIdPair> newNameFileIdPairs = new();

            for (int i = 0; i < uvs.Length; i++)
			{
				bool metaExists = false;

                // Check if the sprite meta is already in the atlas
                for (int ri = 0; ri < spriteRects.Length; ri++)
                {
					SpriteRect spriteRect = spriteRects[ri];

                    if (spriteRect.name == spritesImportData[i].name)
                    {
                        ...
                        spriteRect.rect = currentRect;
                        newSpriteRects.Add(spriteRect);
						if (nameFileIdPairs.Any(p => p.name == spriteRect.name))
						{
							newNameFileIdPairs.Add(nameFileIdPairs.Find(p => p.name == spriteRect.name));
						}
						else
						{
							newNameFileIdPairs.Add(new SpriteNameFileIdPair(spriteRect.name, GUID.Generate()));
				...
				// In case it's a new sprite in the atlas
				if (!metaExists)
				{
                    Rect rect = uvs[i];
					...
					SpriteRect newSprite = new();
					newSprite.name = spritesImportData[i].name;
                    newSprite.spriteID = GUID.Generate();
                    newSprite.rect = rect;
                    newSprite.alignment = spritesImportData[i].alignment;
                    newSprite.pivot = spritesImportData[i].pivot;
                    newSprite.border = spritesImportData[i].border;

                    // Add the Sprite Rect to the array of existing Sprite Rects
                    newSpriteRects.Add(newSprite);

                    // Add the sprite name and GUID to the list of pairs
                    newNameFileIdPairs.Add(new SpriteNameFileIdPair(newSprite.name, newSprite.spriteID));
                }
            }

			spriteNameFileIdDataProvider.SetNameFileIdPairs(newNameFileIdPairs);
		...
		public static Sprite LoadSprite(Texture2D mainTexture, string name)
		{
            ...
            UnityEngine.Object[] atlasAssets = AssetDatabase.LoadAllAssetsAtPath(texturePath);
			
			foreach (UnityEngine.Object asset in atlasAssets)
			{
		...
		/// <param name="obj">Object.</param>
		public static bool HasSubAssets(UnityEngine.Object obj)
		{
		...
		/// <param name="obj">Object.</param>
		public static UnityEngine.Object[] GetSubAssets(UnityEngine.Object obj)
		{
            UnityEngine.Object[] assets = AssetDatabase.LoadAllAssetsAtPath(AssetDatabase.GetAssetPath(obj));
		...
		/// <param name="path">Path.</param>
		public static UnityEngine.Object[] GetDirectoryAssets(string path)
		{
			...
			foreach (string assetPath in assetPaths)
			{
                ...
                UnityEngine.Object objAsset = AssetDatabase.LoadAssetAtPath(assetPath, typeof(UnityEngine.Object));
		...
		public static UnityEngine.Object[] FilterResourcesForAtlasImport(UnityEngine.Object[] resources)
		{
			...
			foreach (UnityEngine.Object resource in resources)
			{
				...
				{
                    UnityEngine.Object[] subAssets = SPTools.FilterResourcesForAtlasImport(SPTools.GetSubAssets(resource));
					
					foreach (UnityEngine.Object a in subAssets) tempList.Add(a);
				}
				...
				{
                    UnityEngine.Object[] subAssets = SPTools.FilterResourcesForAtlasImport(SPTools.GetDirectoryAssets(resourcePath));
		...
		public static int ReplaceReferencesInAllScenes(List<SPSpriteInfo> spriteInfoList, SPReferenceReplacerWindow.ReplaceMode replaceMode, bool spriteRenderersOnly, bool skipCurrent)
		{
            ...
            foreach (EditorBuildSettingsScene scene in EditorBuildSettings.scenes)
			{
				...
				{
					Component[] comps = UnityEngine.Object.FindObjectsByType<Component>(FindObjectsSortMode.None);
		...
		public static int ReplaceReferences(Component[] components, Sprite find, Sprite replaceWith, bool spriteRenderersOnly)
		{
			...
			int count = 0;
			foreach (UnityEngine.Object comp in components)
			{
